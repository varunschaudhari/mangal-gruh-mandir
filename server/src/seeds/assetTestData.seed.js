 ...
 * Creates assets + transactions in all statuses for UI testing.
 *
 * Run AFTER index.js seed:
 *   node src/seeds/index.js
 *   node src/seeds/assetTestData.seed.js
 *
 * Safe to re-run — clears only asset collections before inserting.
 */
...
// ─── Asset master data ───────────────────────────────────────────────────────
const ASSETS = [
  { name: 'PA Speaker System',      category: 'Electronics', totalQuantity: 2, finePerDay: 100, description: '500W portable PA with mic stand' },
  { name: 'Wireless Microphone',    category: 'Electronics', totalQuantity: 4, finePerDay: 50,  description: 'UHF handheld mics' },
  { name: 'Generator (5 KVA)',      category: 'Electronics', totalQuantity: 1, finePerDay: 200, description: 'Petrol generator for outdoor events' },
  { name: 'LED Projector',          category: 'Electronics', totalQuantity: 1, finePerDay: 150, description: '3000 lumen HDMI projector' },
  { name: 'Mandap Set (Full)',       category: 'Mandap',      totalQuantity: 1, finePerDay: 500, description: 'Complete mandap with pillars and canopy' },
  { name: 'Mandap Pillars (Set)',    category: 'Mandap',      totalQuantity: 4, finePerDay: 100, description: 'Decorative brass-finish pillars' },
  { name: 'Steel Plates',           category: 'Utensils',    totalQuantity: 100, finePerDay: 0, description: 'Stainless steel dinner plates' },
  { name: 'Steel Bowls (Katori)',   category: 'Utensils',    totalQuantity: 200, finePerDay: 0, description: 'Small serving bowls' },
  { name: 'Steel Glasses',          category: 'Utensils',    totalQuantity: 150, finePerDay: 0, description: '250ml steel tumblers' },
  { name: 'Large Cooking Vessel',   category: 'Utensils',    totalQuantity: 6, finePerDay: 20,  description: '50-litre aluminium degchi' },
  { name: 'Gas Stove (Double)',      category: 'Utensils',    totalQuantity: 3, finePerDay: 30,  description: 'Double-burner LPG stove' },
  { name: 'Folding Chairs',         category: 'Furniture',   totalQuantity: 50, finePerDay: 5,  description: 'Plastic folding chairs' },
  { name: 'Folding Tables',         category: 'Furniture',   totalQuantity: 20, finePerDay: 10, description: '6-foot rectangular tables' },
  { name: 'Carpet / Dari',          category: 'Decoration',  totalQuantity: 8, finePerDay: 20,  description: '6x4 ft cotton dari' },
  { name: 'Tabla Set',              category: 'Electronics', totalQuantity: 1, finePerDay: 100, description: 'Tabla with bayan for bhajans' },
  { name: 'Harmonium',              category: 'Electronics', totalQuantity: 1, finePerDay: 75,  description: 'Portable harmonium' },
  { name: 'LED Decoration Lights',  category: 'Decoration',  totalQuantity: 10, finePerDay: 15, description: '10m LED string lights' },
  { name: 'Tarpaulin / Shamiyana',  category: 'Furniture',   totalQuantity: 3, finePerDay: 50,  description: '20x30 ft waterproof canopy' },
];
...
async function seedAssetData() {
  ...
  console.log('  Cleared AssetTransaction and Asset collections');
  ...
  for (const u of STAFF_USERS) {
    ...
    if (existing) {
      ...
      const created = await User.create({
        ...u,
        password: 'Test@1234',
        phone: `91900000000${borrowers.length + 1}`,
        isActive: true,
      });
  ...
  // Use first store_manager as secondary approver too
  ...
  for (const a of ASSETS) {
    const asset = await Asset.create({ ...a, createdBy: helpDeskUser._id });
  ...
  // 1. APPROVED — Steel Plates (not yet collected)
  txns.push({
    asset: asset('Steel Plates')._id,
  ...
  txns.push({
    asset: asset('Folding Chairs')._id,
  ...
  txns.push({
    asset: asset('Mandap Set (Full)')._id,
    borrower: borrowers[1]._id,
    quantityBorrowed: 1,
    expectedReturnDate: daysFromNow(7),
    approvedBy: approver2._id,
    approvedAt: daysAgo(1),
    checkedOutAt: daysAgo(0),
    status: 'checked_out',
    conditionAtCheckout: 'good',
    notes: 'Wedding ceremony at staff member home',
    createdBy: helpDeskUser._id,
  });

  // 6. OVERDUE — Generator (3 days overdue)
  txns.push({
    asset: asset('Generator (5 KVA)')._id,
    borrower: borrowers[2]._id,
    quantityBorrowed: 1,
  ...
  txns.push({
    asset: asset('Steel Bowls (Katori)')._id,
    borrower: borrowers[1]._id,
  ...
  txns.push({
    asset: asset('Harmonium')._id,
    borrower: borrowers[0]._id,
    quantityBorrowed: 1,
    expectedReturnDate: daysAgo(15),
    actualReturnDate: daysAgo(13),
    approvedBy: approver._id,
  ...
  txns.push({
    asset: asset('Carpet / Dari')._id,
    borrower: borrowers[1]._id,
    quantityBorrowed: 3,
    expectedReturnDate: daysAgo(20),
    actualReturnDate: daysAgo(18),
    approvedBy: approver._id,
    approvedAt: daysAgo(28),
    checkedOutAt: daysAgo(27),
    status: 'returned',
    conditionAtCheckout: 'good',
  ...
  txns.push({
    asset: asset('Folding Tables')._id,
    borrower: borrowers[2]._id,
    quantityBorrowed: 10,
    ...
    fineApplied: false,
    extensions: [{
      previousReturnDate: daysAgo(7),
      newReturnDate: daysAgo(2),
      approvedBy: approver._id,
      approvedAt: daysAgo(8),
      extendedBy: helpDeskUser._id,
      notes: 'Event extended by 5 days',
    }],
    createdBy: helpDeskUser._id,
  });
  ...
  txns.push({
    asset: asset('LED Decoration Lights')._id,
    borrower: borrowers[0]._id,
    quantityBorrowed: 5,
    expectedReturnDate: daysFromNow(2),
    approvedBy: approver._id,
    approvedAt: daysAgo(1),
    status: 'cancelled',
    cancellationReason: 'Event postponed due to rain',
    createdBy: helpDeskUser._id,
  });
